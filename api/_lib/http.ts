...
export function sendError(res: any, error: unknown): void {
  ...
  if (errorCode === 'AI_TIMEOUT') {
    ...
    message = 'Unable to reach DashScope API (network timeout). Please check outbound network access and try again.';
  } else if (causeCode === 'ENOTFOUND') {
    message = 'Unable to resolve DashScope API host (DNS failure). Please check your network/DNS settings.';
  } else if (causeCode === 'ECONNREFUSED') {
    message = 'Connection to DashScope API was refused. Please check network proxy or firewall settings.';
  }
