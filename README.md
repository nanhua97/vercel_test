...
- AI：瀏覽器直接呼叫 Qwen（DashScope OpenAI 相容接口）
- 報告：前端渲染 + 前端匯出 PDF
...
1. 匯入專案到 Vercel。
2. 在專案環境變數加入 `VITE_QWEN_API_KEY`（以及需要的選填變數）。
3. Deploy。

## 注意事項

- 本專案採前端直連 Qwen，`VITE_QWEN_API_KEY` 會出現在前端資產中，請務必搭配 DashScope 的限額與金鑰限制策略。
